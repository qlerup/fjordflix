...
function setupCategories() {
  ...
  section.insertAdjacentHTML('afterbegin', `<aside id="library-categories" class="library-categories" hidden aria-labelledby="categories-title">
    ...
    <span class="sidebar-section-label">BIBLIOTEK</span><nav id="sidebar-browse" aria-label="Bibliotek">
    <button class="sidebar-view" data-side-view="home">Hjem</button><button class="sidebar-view" data-side-view="all">Film</button>
    <button class="sidebar-view" data-side-view="series">Serier</button><button class="sidebar-view" data-side-view="favorites">Min liste</button></nav>
...
function renderCategories(items, availableItems = items) {
  ...
  const genresVisible = ['all', 'series'].includes(view);
  ...
  $('categories-title').textContent = view === 'series' ? 'SERIEGENRER' : 'FILMGENRER';
