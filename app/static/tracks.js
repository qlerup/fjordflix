const FjordTracks = {
  movie: null, audio: null, subtitle: null, generation: 0, element: null, url: null, pending: null,
  language(code) {
  ...
  async prepare(movie) {
    this.movie = movie; this.audio = null; this.subtitle = null;
  ...
  sync() { for (const place of ['detail','player']) for (const kind of ['audio','subtitle']) $(`${place}-${kind}`).value = this[kind] ?? ''; },
  selectedSubtitle() { return this.movie?.tracks?.subtitles?.find(t => t.index === this.subtitle); },
  request() { return {audio_track:this.audio, subtitle_track:this.subtitle}; },
  clear() {
    ++this.generation;
