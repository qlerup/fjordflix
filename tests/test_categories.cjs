...
test('persistent sidebar filters without closing and remembers film and series selections', () => {
  ...
  assert.equal(w.$('sidebar-library').hidden,false);
  assert.equal(w.$('sidebar-back').hidden,true);
  ...
  header.innerHTML = '<button data-view="all">Film</button><button data-view="home">Hjem</button>';
  w.document.body.append(header);
  header.firstChild.onclick = () => {w.view = 'all'; w.render();};
  header.lastChild.onclick = () => {w.view = 'home'; w.render();};
  w.document.querySelector('[data-side-view="all"]').click();
  assert.equal(w.view,'all');
  assert.equal(w.$('sidebar-library').hidden,true);
  assert.equal(w.$('sidebar-back').hidden,false);
  assert.equal(w.$('sidebar-genres').hidden,false);
  assert.equal(result.label,'Komedie');
  w.$('sidebar-back').click();
  assert.equal(w.view,'home');
  assert.equal(w.$('sidebar-library').hidden,false);
  assert.equal(w.document.activeElement.dataset.sideView,'home');
  w.view = 'favorites'; w.render();
  assert.equal(w.$('sidebar-library').hidden,true);
  assert.equal(w.$('sidebar-back').hidden,false);
  assert.equal(w.$('sidebar-genres').hidden,false);
  assert.equal(links.firstChild.textContent,'Alle på min liste');
  choose('drama');
  assert.equal(result.items.length,1);
  assert.equal(result.label,'Drama');
  w.renderCategories([], []);
  assert.equal(links.children.length,1);
  assert.equal(links.firstChild.getAttribute('aria-current'),'page');
  dom.window.close();
});
