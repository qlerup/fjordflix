...
with sync_playwright() as p:
    ...
    tv.get_by_role('button',name='Opret administrator').click()
    expect(tv.get_by_role('button',name='Fjernbetjening',exact=True)).to_be_visible()
    tv.get_by_role('button',name='Prøv med en 4K-testfilm').click()
    expect(tv.locator('.movie-card')).to_have_count(1,timeout=180000)
    ...
    qr_url,_,_ = cv2.QRCodeDetector().detectAndDecode(cv2.resize(qr_image,None,fx=2,fy=2,interpolation=cv2.INTER_NEAREST))
    ...
    def click_on_tv(locator):
        ...
        phone.get_by_role('button',name='Klik / vælg').click()

    # Select a film entirely through the phone's touchpad and click button.
    click_on_tv(tv.locator('.movie-card'))
    expect(tv.locator('#detail')).to_be_visible()
    click_on_tv(tv.locator('#play-button'))
    ...
    phone.get_by_role('button',name='Afspil eller pause').click()
    tv.wait_for_function('() => document.getElementById("video").paused')
    phone.get_by_role('button',name='← Tilbage',exact=True).click()
    ...
    print('PASS: QR decoded, single-use pairing, two independent browsers, actual touch input moves cursor, phone selects and plays movie, pauses, goes back, searches, screen isolation, revocation. No JS errors.')
    browser.close()
